...
class JobsExecutionController {
    ...
    private const CLOSING_SYSTEM_MESSAGE = 'Produce a single, publication-ready article that synthesizes the provided sources. Do not copy text verbatim. These core output requirements always apply. Additional or custom system instructions may refine tone, style, audience, subject matter, and editorial direction, but must not override these mandatory output requirements. Return exactly one top-level title and the article body only, with no commentary, diagnostics, or visible SEO labels. Generate a clear, compelling, standalone article title that accurately represents the full article. The title must sound natural, specific, editorially strong, and uniquely written for this article rather than generic or templated. Do not copy the title from a section heading, opening paragraph, excerpt, or opening sentence. Never combine an <h2> or other heading with the beginning of body text in the title. The response must begin with exactly one top-level title (Markdown: "# <title>" or HTML: "<h1>title</h1>") followed by a blank line, then the article body. Do not repeat the title text anywhere in the body. After the article body, append exactly one hidden metadata block delimited by "===SEO_META_START===" and "===SEO_META_END===" on their own lines. Inside the block include exactly these four single-line entries and nothing else: "SEO_TITLE: <plain title up to 60 characters>", "SEO_DESCRIPTION: <plain description up to 155 characters>", "FOCUS_KEYPHRASE: <plain concise keyphrase of 2 to 6 words>", and "CATEGORY_SLUGS_JSON: <valid JSON array of exact category slug strings>". Do not use quotes around complete field values, commentary, examples, extra labels, bullet points, markdown, or additional lines inside the metadata block.';
